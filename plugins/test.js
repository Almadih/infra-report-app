...
function renameMetadata(androidManifest, existingName, newName) {
  ...
  const hasExistingNameEntry =
    metaData.findIndex((meta) => meta["$"]["android:name"] === existingName) !==
    -1;

  if (hasExistingNameEntry) {
    // Remove all entries with new name
    const newNameMetaDataEntriesIndices = metaData.reduce(
      (acc, meta, index) => {
...
module.exports = function withRenameMetadata(
  config,
